-- grantz: authorization schema, MySQL, bigint user ids.
--
...
--   001_init_mysql.sql           MySQL 8.0.19+, bigint user ids   <- this file
...
--
-- Pair it with sqlstore.NewMySQL(db) and grantz.New(...). MySQL 8.0.19 is the floor, for
-- the row-alias upsert the store's Sync uses.
--
...
--
-- What differs from the Postgres file, and why:
--
--   * jsonb          -> json. Same storage decision, MySQL's spelling.
--   * serial         -> int AUTO_INCREMENT. Not MySQL's SERIAL, which is a bigint unsigned.
--   * timestamp      -> datetime. MySQL's timestamp stops in 2038 and shifts with the
--                       session time zone; datetime does neither.
--   * `key`          is quoted, because KEY is reserved in MySQL. The column keeps its
--                       name so both schemas describe the same thing.
--   * inline REFERENCES -> table-level FOREIGN KEY. MySQL PARSES a column-level REFERENCES
--                       clause and then silently ignores it, which is the worst of both:
--                       the migration succeeds and the constraint does not exist.
--   * CREATE INDEX IF NOT EXISTS -> indexes declared inside the table. MySQL has no
--                       IF NOT EXISTS for indexes, and a re-run must not fail.
--   * ENGINE=InnoDB  is explicit, because MyISAM accepts foreign keys and ignores them.
--   * utf8mb4        is explicit, because a foreign key between varchar columns requires
--                       both sides to share a character set, and defaults vary by server.
